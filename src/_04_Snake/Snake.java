...
public class Snake {
	...
	public void update() {
		 ...
		 */
		int nextX = getHeadLocation().getX(), nextY = getHeadLocation().getY();
	...
	public boolean isOutOfBounds() {
		...
		if (loc.getX() >= SnakeGame.WIDTH || loc.getX() < 0 || loc.getY() >= SnakeGame.HEIGHT || loc.getY() < 0)
		{
			return true;
		}
		else
	...
	public boolean isHeadCollidingWithBody() {
		...
		for (int i = snake.size() - 1; i > 0; i--)
		{
			if (headLoc.positionsMatch(snake.get(i).getLocation()))
			{
				return true;
			}
		}
	...
	public boolean isLocationOnSnake(Location loc) {
		...
		{
			if (loc.positionsMatch(snake.get(i).getLocation()))
			{
				return true;
			}
		}
