...
public class IntroToEnums {
	...
	public static void main(String[] args) {
		...
		StatesOfMatter[] statesOfMatterValues = StatesOfMatter.values();
		// 4. Ask the user for a state of matter.
		String input = JOptionPane.showInputDialog("Input a state of matter");
		// 5. Iterate through the array and find what the user entered.
		// Hint: .name() or .toString
		StatesOfMatter inputedState = null;
		for (StatesOfMatter i : statesOfMatterValues)
		{
			if (i.toString().equals(input.toUpperCase()))
			{
				inputedState = i;
		...
		// Hint: .ordinal()
		if (inputedState != null)
		{
			System.out.println(inputedState.toString());
			System.out.println(inputedState.ordinal());
		}
		...
		StatesOfMatter randState = statesOfMatterValues[new Random().nextInt(3)];
		// 11. Print outs both of its temperatures.
		System.out.println(randState.getStateTemp());
		System.out.println(randState.convertFahrenheit(randState.getStateTemp()));
